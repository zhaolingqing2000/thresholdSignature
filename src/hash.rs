...
/// H_com(i || rho_i || B_i) -> mu_i for the original Gargos round-1 commitment.
...
/// Compatibility wrapper for older call sites. The original Gargos commitment
/// hash intentionally excludes sid, signer-set, and message; those values are
/// bound later by the transcript-level auxiliary layer.
pub fn hbind(
    _sid: &[u8; 32],
    _signer_set: &[u32],
    _message: &[u8],
...
/// H_sig(A_hat || X || m) -> Z_q for the final Gargos challenge.
...
/// Compatibility wrapper for older call sites. The challenge follows the
/// original Gargos signing semantics and therefore excludes sid and S.
pub fn hsig_bound(
    _sid: &[u8; 32],
    _signer_set: &[u32],
