...
const FAST_TRIALS: usize = 100;
const CONCRETE_TRIALS: usize = 20;
const RELATION_TRIALS: usize = 5;
const CORRECTNESS_TRIALS: usize = 20;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    append_interface_and_partial_notes(&mut report);
    ...
    fs::write("results/partial_concrete_evaluation.md", report)?;
    Ok(())
}
...
fn environment() -> Vec<(String, String)> {
    vec![
        ...
        (
            "Cargo features".into(),
            "concrete-lhtlp, concrete-ibe; no default features".into(),
        ),
    ]
}
...
fn run_ibe() -> Result<IbeReport, Box<dyn std::error::Error>> {
    let setup = stats(CONCRETE_TRIALS, || {
        let _ = ConcreteIbeBackend::setup();
    });
    ...
    let d = [7u8; 32];
    let wrong_d = [8u8; 32];
    let extract = stats(CONCRETE_TRIALS, || {
    ...
    let encrypt = stats(CONCRETE_TRIALS, || {
        let _ =
    ...
    let ct = ConcreteIbeBackend::encrypt_with_randomness(&pk, message, &d, 1, &randomness)
        .map_err(|e| format!("{e:?}"))?;
    let decrypt = stats(CONCRETE_TRIALS, || {
...
fn append_header(report: &mut String, env: &[(String, String)], elapsed: Duration) {
    report.push_str("# Partial Concrete Evaluation\n\n");
...
fn append_sp1(report: &mut String) {
    report.push_str("## Previously Observed SP1 Execution Information\n\n");
...
fn append_interface_and_partial_notes(report: &mut String) {
    report.push_str("## Interface-Level Gargos-versus-Auxiliary Comparison\n\n");
    report.push_str("Mode: `prototype-level comparison using interface-compatible backends`.\n\n");
    report.push_str("This comparison is not measured in this concrete-only run. The required command uses `--no-default-features --features concrete-lhtlp,concrete-ibe`, so the repository's `test-backend` interface-level prototype is intentionally not active. Mixing test-backend prototype measurements into this concrete run would combine incompatible backends in one table. No prototype values are estimated or copied into this report.\n\n");
    report.push_str("## Partial-Concrete Auxiliary Path Excluding NIZK\n\n");
    report.push_str(
        "Mode: `partial-concrete data-flow latency excluding NIZK proving and verification`.\n\n",
    );
    report.push_str("The concrete data-flow checks in this run use concrete LHTLP and concrete IBE, and the object-size accounting below uses actual canonical serialization. Stage-level partial-concrete latency for `SignEncap`, `Combine`, `Open`, `Verify`, and `Trace` is not measured as a separate distribution in this run. The correctness table reports the measured data-flow acceptance behavior; it must not be interpreted as complete cryptographic protocol acceptance because no NIZK proof is generated or verified.\n\n");
    report.push_str("## Pedersen Commitment Results\n\n");
    report.push_str("Pedersen commitment microbenchmarks are not measured in this run. Commitment objects are included in the canonical handle and record sizes below. This report does not estimate commitment generation, opening verification, or multi-commitment aggregation latency.\n\n");
}
...
fn append_limitations(report: &mut String, elapsed: Duration) {
    report.push_str("## Paper-Ready Tables\n\n");
    report.push_str(
        "The following tables can be transcribed into LaTeX after shortening column labels:\n\n",
    );
    report.push_str("- Concrete LHTLP timing: mode `concrete-lhtlp`, parameters `3072-bit N`, samples `20`, excludes NIZK.\n");
    report.push_str("- Concrete IBE timing: mode `concrete-ibe`, backend `CGWFO KEM + AES-256-GCM`, samples `20`, excludes NIZK.\n");
    report.push_str("- Native binding relation validation: mode `native full-relation validation`, samples `5`, not a NIZK proof.\n");
    report.push_str(
        "- Gargos baseline: mode `Gargos baseline`, samples `100`, no auxiliary objects.\n",
    );
    report.push_str("- Serialization overhead: mode `partial concrete excluding NIZK`, proof size `not measured`.\n\n");
    report.push_str("## Recommended Paper Wording\n\n");
    report.push_str("The implementation currently reaches a `PARTIAL CONCRETE INSTANTIATION`: the LHTLP and tracing backends are concrete and measured with repeated trials, while the full cross-layer binding relation is implemented and validated natively. A previous SP1 guest execution of the complete relation reported 9,306,358,310 guest instructions, but this evaluation does not generate a NIZK proof. Consequently, all partial-concrete latency and size results exclude NIZK proving cost, NIZK verification cost, and proof bytes, and should not be presented as production-level performance for the complete concrete construction.\n\n");
    ...
    report.push_str("```text\nRUSTFLAGS=\"-C target-cpu=native\" cargo run --release -p threshold_signature --no-default-features --features concrete-lhtlp,concrete-ibe --bin partial_concrete_evaluation\n```\n\n");
    report.push_str(&format!(
